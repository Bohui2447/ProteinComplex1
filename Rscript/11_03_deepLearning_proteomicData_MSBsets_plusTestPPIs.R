...
# 11_02 only use validation set to evalute model performance

## 11_03 plus test set ppis to re-caculate model performance
...
if(file.exists('../Rdata/11_03_exprAllDivede_MSBTest_Combine_TextValidation.Rdata')){
  cat('load ../Rdata/11_03_exprAllDivede_MSBTest_Combine_TextValidation.Rdata')
  load('../Rdata/11_03_exprAllDivede_MSBTest_Combine_TextValidation.Rdata')
}else {
  ...
  save(testNorm_combine, test_combineLables, 
       file = '../Rdata/11_03_exprAllDivede_MSBTest_Combine_TextValidation.Rdata')
}
...
fl = list.files('../Rdata/ModelSave/11_02/model/')
for(ni in fl[74:length(fl)]){
  print(ni)
  model = load_model_hdf5(paste0('../Rdata/ModelSave/11_02/model/', ni))
  ...
  if (F1_value > 0.49){
    ...
    mem_used2(T)
    rm(parameter, F1_recall_precision, 
       auc , roc ,  
       prec_rec_plot ,
       pre_prob , pred , 
       pre_prob_trainSet,
